...
*/
#include "InverseDynamicsIRB120Example.h"
...
class InverseDynamicsIRB120Example : public CommonMultiBodyBase
{
    btInverseDynamicsIRB120ExampleOptions m_option;
    btMultiBody* m_multiBody;
    btInverseDynamics::MultiBodyTree *m_inverseModel;
    TimeSeriesCanvas* m_timeSeriesCanvas;
public:
    InverseDynamicsIRB120Example(struct GUIHelperInterface* helper, btInverseDynamicsIRB120ExampleOptions option);
    virtual ~InverseDynamicsIRB120Example();
...
InverseDynamicsIRB120Example::InverseDynamicsIRB120Example(struct GUIHelperInterface* helper, btInverseDynamicsIRB120ExampleOptions option)
...
InverseDynamicsIRB120Example::~InverseDynamicsIRB120Example()
{
    delete m_multiBody;
...
void InverseDynamicsIRB120Example::initPhysics()
{
   ...
   // gravity[upAxis]=-9.8;
    ...
    switch (m_option)
    {
    case BT_IDI_LOAD_URDF:
        {
            BulletURDFImporter u2b(m_guiHelper);
            bool loadOk =  u2b.loadURDF("abb_irb120/irb120.urdf");
            if (loadOk)
            {
    ...
    default:
        {
            b3Error("Unknown option in InverseDynamicsIRB120Example::initPhysics");
            b3Assert(0);
        }
    };
...
void InverseDynamicsIRB120Example::stepSimulation(float deltaTime)
{
    if(m_multiBody) {
...
CommonExampleInterface*    InverseDynamicsIRB120ExampleCreateFunc(CommonExampleOptions& options)
{
    return new InverseDynamicsIRB120Example(options.m_guiHelper, btInverseDynamicsIRB120ExampleOptions(options.m_option));
}
